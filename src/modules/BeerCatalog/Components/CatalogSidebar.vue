...
<template>
  <div class="flex justify-between border-b-2 border-b-gray-200">
    ...
    <button id="dropdownRadioButton" data-dropdown-toggle="dropdownDefaultRadio" class="md:hidden w-16 p-2 my-2 mx-3 text-sm text-gray-500 rounded-lg bg-gray-200 text-center inline-flex items-center focus:outline-none focus:ring-2 focus:ring-gray-200" type="button">
      Sort
      ...
      <ul class="p-3 space-y-3 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownRadioButton">
        <li>
          <div class="flex items-center">
            ...
            <label for="default-radio-1" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Rating</label>
          </div>
        </li>
        <li>
          <div class="flex items-center">
            ...
            <label for="default-radio-2" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Name</label>
          </div>
        </li>
      </ul>
    </div>
  </div>
  <aside id="filter-sidebar" class="fixed top-16 left-0 z-40 w-3/4 md:w-64 h-screen transition-transform -translate-x-full md:static md:translate-x-0" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 border-r-2  dark:bg-gray-800">
      <ul class="space-y-2 font-medium pt-5">
        <li>
          <h2 class="text-lg font-primary font-bold text-hopix-gray">Sort by:</h2>
          <ul class="overflow-y-auto pt-2 h-16 w-3/4 border-t-2 px-1">
            <li>
              <div class="flex items-center">
                ...
                <label for="b" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Rating</label>
              </div>
            </li>
            <li>
              <div class="flex items-center">
                ...
                <label for="a" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Name</label>
              </div>
            </li>
          </ul>
        </li>
        <li class="pb-5">
          <h2 class="text-lg font-primary font-bold text-hopix-gray">Brewery</h2>
          <ul class="overflow-y-auto h-36 w-3/4 border-y-2 px-1 whitespace-nowrap overflow-x-hidden text-ellipsis">
            <li v-for="brewery in breweries">
              <input :id="brewery.id" type="checkbox" :value="brewery.name" v-model="selectedBreweries" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
              <label :for="brewery.id" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{brewery.name}}</label>
            </li>
          </ul>
        </li>
        <li>
          <h2 class="text-lg font-primary font-bold text-hopix-gray">Aroma</h2>
          <ul class="overflow-y-auto h-36 w-3/4 border-y-2 px-1 whitespace-nowrap overflow-x-hidden text-ellipsis">
            <li v-for="aroma in aromas">
              <input :id="aroma.id" type="checkbox" :value="aroma.name" v-model="selectedAromas" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
